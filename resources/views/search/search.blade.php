            ...
            <form action="{{ url('/') }}" method="POST">
                ...
                <div class="row">
                    ...
                    <div class="col-12 col-lg-3">
                        <label for="cities" class="form-label mb-0 mt-2 mb-md-1">Max. czas podróży <span @popper(Podaj w
                                                                                                         minutach
                                                                                                         maksymalny czas
                                                                                                         dojazdu do
                                                                                                         lokalizacji
                                                                                                         docelowej)><x-codicon-info
                                        class="icon-info"/></span></label>
                    ...
                    <div class="col-12 col-md-6 col-xl-4 mb-4">
                        <div class="card h-100">
                            <ul class="nav nav-pills" id="pills-tab" role="tablist">
                                <li class="nav-item" role="presentation">
                                    <button class="btn btn-custom-outline active"
                                            id="pills-home-tab-{{ $flat['rand'] }}" data-bs-toggle="pill"
                                            data-bs-target="#pills-home-{{ $flat['rand'] }}" type="button" role="tab"
                                            aria-controls="pills-home-{{ $flat['rand'] }}" aria-selected="true">Zdjęcie
                                    </button>
                                </li>
                                <li class="nav-item" role="presentation">
                                    <button class="btn btn-custom-outline border-button"
                                            id="pills-profile-tab-{{ $flat['rand'] }}" data-bs-toggle="pill"
                                            data-bs-target="#pills-profile-{{ $flat['rand'] }}" type="button" role="tab"
                                            aria-controls="pills-profile-{{ $flat['rand'] }}" aria-selected="false">Mapa
                                    </button>
                                </li>
                            </ul>

                            <div class="tab-content" id="pills-tabContent">
                                <div class="tab-pane fade show active" id="pills-home-{{ $flat['rand'] }}"
                                     role="tabpanel" aria-labelledby="pills-home-tab-{{ $flat['rand'] }}">
                                    <img src="{{ $flat['photo'] }}" class="search-img" alt="">
                                </div>
                                <div class="tab-pane fade" id="pills-profile-{{ $flat['rand'] }}" role="tabpanel"
                                     aria-labelledby="pills-profile-tab-{{ $flat['rand'] }}">

                                    <iframe class="card-img-top" width="100%" height="350"
                                            src="https://maps.google.com/maps?q={{ $flat['localization'] }}&output=embed&z=12"></iframe>
                                </div>
                            </div>
                            <div class="card-body pb-0 pt-1">
                                <h5 class="col-12 mt-2 card-title fs-6">{{ $flat['localization']}}</h5>
                                <h5 class="col-12 mt-2 mb-3 card-title fs-5 fw-bold price"><b>{{ $flat['price'] }}</b>
                                </h5>
                            </div>
                            ...
                            <div class="row m-0 p-0">
                                <a href="{{ $flat['url'] }}" target="_blank"
                                   class="mt-4 btn btn-custom-solid btn-cards">Otwórz w nowej karcie</a>
                                <div class="btn-heart_active--wrapper">
                                    <form id="StoreFav-{{ $flat['rand'] }}">
                                        @csrf
                                        <input type="hidden" value="{{$flat['localization']}}" name="localization">
                                        ...
                                        <button id="{{ $flat['rand'] }}" class="mt-4 btn btn-heart_add">
                                            <x-clarity-heart-solid/>
                                        </button>
                                    </form>
                                    <script>
                                        window.favs_store = "{{ route('favs.store') }}";
                                    </script>
                                </div>
                            </div>
                            <script>
