...
<head>
    ...
    <title>Las Mieszkanias - Twoja wyszukiwarka mieszkań</title>
...
<div id="app" class="{{ Route::is('login') || Route::is('register') ? "center-mobile" : ""}}">
    <nav class="navbar navbar-expand-md navbar-dark bg-dark main-nav"
         style="{{ Route::is('login') || Route::is('register') ? "display: none;" : ""}}">
                ...
                <ul class="navbar-nav ms-auto mb-2 mb-md-0 ">
                    @guest
                        @if (Route::has('login'))
                            <li class="nav-item me-md-3 my-3 my-md-0">
                                <a class="btn-custom-outline_white nav-link "
                        ...
                        @if (Route::has('register'))
                            <li class="nav-item">
                                <a class="nav-link btn-custom-solid_white m-auto"
                    ...
                    @else
                        ...
                        <li class="nav-item me-md-3 my-3 my-md-0 d-flex align-items-center">
                            <a class="btn-icon-fill_white {{-- if user has favs change color to red --}}"
                               href="{{ route('favs.index') }}">
                                <x-ri-heart-fill class="icon-avatar icon-heart"/>
                            </a>
                        </li>
                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle d-flex align-items-center text-white" href="#"
                               id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                                <x-radix-avatar class="icon-avatar me-1"/>
                                <span>{{ Auth::user()->name }}</span>
                            </a>
                            <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="navbarDropdown">
                                <li><a class="dropdown-item" href="{{ route('favs.index') }}">Ulubione</a></li>
                                <li><a class="dropdown-item" href="{{ route('filters.index') }}">Zapisane filtry</a>
                                </li>
